...
export default function Footer() {
  ...
  const socialLinks = [
    ...
    { 
      name: "LinkedIn", 
      href: "https://www.linkedin.com/in/claire-niyigena-89784a248/", 
      icon: (
        <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex gap-4">
              {socialLinks.map((link) => (
                <Link
                  ...
                  className="text-[#f8fafc]/60 hover:text-[#f59e0b] transition-colors"
                  title={link.name}
                >
                  {typeof link.icon === 'string' ? <span className="text-3xl">{link.icon}</span> : link.icon}
                </Link>
              ))}
            </div>
          </div>
